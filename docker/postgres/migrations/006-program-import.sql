...
-- Флаг: детализация по дому из программы капитального ремонта успешно загружена из внешнего API.
-- Существующие строки считаются незагруженными; очистка данных программ флаг не сбрасывает.
ALTER TABLE public.repair_houses
    ADD COLUMN IF NOT EXISTS program_data_loaded boolean NOT NULL DEFAULT false;

-- Таблицы программного импорта (для томов, созданных до появления этой фичи).
